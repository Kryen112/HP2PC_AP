...
- **Durable filler-bean grants via per-item index tracking.** v1 limitation: `Small / Medium / Large Pile of Beans` are excluded from the sidecar's `durable_grants[]` because `managerStatus.AddBeans(N)` is non-idempotent — replaying a bean grant the player already saved would double the count. Consequence: beans received between save and quit are lost (cards/spells are unaffected — `SetCardOwner` and `AddToSpellBookByString` are idempotent and self-heal via AP's `ReceivedItems` replay). v2 fix: introduce a `consumed_grant_index` on `APCardWatcher` (class default, instance-copied into `.usa` at save / restored on load); add an `APPLIED <index>` mod→sidecar ack so the sidecar knows the high-water mark; on game HELLO the mod sends its restored index so the sidecar replays only `index > N`. Then beans become durable like everything else, and the same machinery solves the "save-load vs new-game distinction" sub-question below (the saved index disambiguates "fresh save with same slot" from "different slot entirely").
