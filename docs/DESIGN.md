...
[HP2 game on M212 engine]
  └─ HPArchipelago.u  (UnrealScript mod)
       └─ class'IpDrv.TcpLink'  →  localhost:38281
                                          │
                                          ▼
                           [Python client — client/hp2_client.py]
                                          │
                                          ▼ WebSocket
                              [archipelago.gg server]
```

**No native code.** All game-side logic is UnrealScript. Python client handles AP WebSocket. This was the architecture once we confirmed M212's engine has TcpLink available; before that, file-IPC was the fallback plan.
...
Implementation: Phase A (filter — clear vendor ownership for AP-checked locations), Phase B (replacement — swap vendor-spawned vanilla `WCXxx` to `APCardMarker_<X>` before pickup), and Phase C (assignment — mirror vanilla `AssignVendorCards` using marker `CardLocationId` instead of the sentinel `Default.Id=200`). gen_apworld.py bakes per-card `bVendorsCanSell` / `strVendorOwnedAfterGState` / tier hints from each vanilla `WCXxx` into the generated marker subclasses. See `docs/MOD_TODO.md` for the full breakdown.
...
- Client's `granted_card_game_ids` dedup is removed — the watcher no longer fires echo CHECKs (its transition path is suppressed by `MarkAsGranted` before `SetCardOwner` runs), so `CHECK <id>` only originates from a real player walk-over of a marker.
...
- **Durable filler-bean grants via per-item index tracking.** v1 limitation: `Small / Medium / Large Pile of Beans` are excluded from the client's `durable_grants[]` because `managerStatus.AddBeans(N)` is non-idempotent — replaying a bean grant the player already saved would double the count. Consequence: beans received between save and quit are lost (cards/spells are unaffected — `SetCardOwner` and `AddToSpellBookByString` are idempotent and self-heal via AP's `ReceivedItems` replay). v2 fix: introduce a `consumed_grant_index` on `APCardWatcher` (class default, instance-copied into `.usa` at save / restored on load); add an `APPLIED <index>` mod→client ack so the client knows the high-water mark; on game HELLO the mod sends its restored index so the client replays only `index > N`. Then beans become durable like everything else, and the same machinery solves the "save-load vs new-game distinction" sub-question below (the saved index disambiguates "fresh save with same slot" from "different slot entirely").
