...
6. ~~**Card-grant persistence into the album**~~ **RESOLVED 2026-05-07.** M212 Discord answer pointed at `WizardCardIcon.uc:131` — vanilla `Touch` ends with `sgCards.RemoveHarryOwnedCardsFromLevel(self)` which is the level-cleanup pass. Original implementation `Spawn(card)` + `cardActor.Touch(harry)` short-circuited at the `CanPickupNow` guard (`bPickupOnTouch`/state checks fail on a freshly-spawned, still-falling actor) so line 131 — and sometimes `SetCardOwner` itself — never ran. Fix: `APGameInfo.TryApplyCard` now writes `siCard.SetCardOwner(cardClass.default.Id, CardOwner_Harry)` directly and calls `sgCards.RemoveHarryOwnedCardsFromLevel(None)` to clean up duplicates in-level. No spawn, no Touch, no preconditions. The previous "HP2 wipes `WizardCards[]`" theory was wrong — `WizardCards[50]` is the canonical store and persists. Pickup FX deferred to M8 polish.
7. **Spell start-state vs randomization** (raised 2026-05-07 during M5 testing). HP2 grants **Lumos, Flipendo, Alohomora** via early cutscenes (Privet Drive escape, intro sequences) that the player can't avoid. If the AP seed places those spells at distant locations, the player still has them in-game from the cutscene but AP's logic doesn't know it. Three options: (a) pre-place them at sphere-0 starting checks so the seed treats them as "start inventory"; (b) suppress those cutscene grants via UScript hook so the player only gets the spells AP grants them; (c) declare the three "intro spells" non-progression for v1 logic so they don't gate solvability. Decide before M6 (logic + seed gen).
