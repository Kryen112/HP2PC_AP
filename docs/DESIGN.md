...
- **True intercept on card pickup (replaces vanilla Touch chain entirely).** v1 implementation lets the vanilla `WizardCardIcon.Touch` chain run, then the watcher reverts the resulting `SetCardOwner(id, Harry)` 0.25s later. This works for the album, but the celebration cutscene at the 10th/20th/etc. bronze-card boundary fires inside that window and grants Harry a permanent +max-HP via `harry.CelebrateCardSet` state's 1.5s-into-`begin:` `IncrementCountPotential` call. The revert happens after the stamina is already applied, and on level re-entry the icon respawns from the .unr so the player can re-pick it up for another stamina. Bounded exploit (max 9 free HP-icons total: 5 from bronze, 4 from silver), doesn't break seed correctness, but is a known v1 limitation. v2 fix: replace each `WizardCardIcon` actor at level entry with an `APCardInterceptor` that overrides `Touch` to send CHECK only — no `SetCardOwner`, no `FancySpawn`, no celebration, no exploit. Also requires intercepting chest-spawned cards (chest's `EjectedObjects` references card classes; would need to swap to interceptor classes or hook the chest spawn). Note: this v2 architecture is also planned for replacing every chest's vanilla card with an AP-exclusive item, so the stamina exploit goes away as a side effect of that work — not a separate fix.
